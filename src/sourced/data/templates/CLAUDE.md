...
| editing → formatting | revision report + §4 audit clean (zero unresolved flagged rows) + citation-payload re-read list clean (zero unresolved `fidelity-drift` rows) + voice audit surface-scan report emitted (empty or `mark as intentional`) + paste target named | revision report, §4 audit list, citation-payload re-read list, voice audit surface-scan report |
...
- **Citation-payload re-read list.** One row per citation instance in the edited prose, each row carrying `@id`, the draft sentence verbatim, the `exact_quote` span (verbatim from the log, plus the relevant `surrounding_context` span where scope hinges on neighbors), and a verdict of `fidelity-hold` or `fidelity-drift: <what the prose asserts that the payload does not support>`. Read off the log payload independently, not from any upstream self-audit, prose plan, or drafter report; the quoted spans make each verdict spot-checkable. Emitted by `[editing]` Pass 2; it is the evidence the §4 items 1, 2, 4, 5 verdicts are read from. A `fidelity-drift` row closes the gate until resolved. Gate: `editing → formatting`.
