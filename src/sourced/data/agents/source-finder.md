...
3. **Verify each candidate before logging:**
   ...
   - **Full-text availability**: readable PDF or rendered HTML of the full work, or the complete OCR text of a scanned work. An abstract is not enough. A paywall you can't pass is not enough. A page that cites the work is not the work. A search-inside snippet window is not the work either: it shows a match, not the text around it, and nothing logged from one can reach `verified`. Record which access you had in `retrieval.access_mode` (step 5).
4. **Read the relevant passage in full and keep the rendered view open for the verification and context traces in step 5.** No logging based on abstracts, titles, reviews, or secondary paraphrases.
5. **Produce a verification trace before marking `verification_status: "verified"`.** Populate these `retrieval` sub-fields in the same pass as `exact_quote`, copying verbatim from the rendered view:
   ...
   - `context_trace`: `{"first_20": "...", "last_20": "..."}`, the first and last 20 characters of your `surrounding_context` exactly as they appeared in the rendered view. `surrounding_context` is verbatim neighbor text from the same rendered view as `exact_quote`, containing the quote; a note about what you could not retrieve is not context, and an entry that needs one is an entry you cannot verify. Reject instead.
   - `access_mode`: which rendered view the copies above came from: `rendered-html`, `pdf-render`, or `ocr-fulltext`. If the best you reached is `snippet` (a search-inside or keyword-in-context window), you do not have full text: reject under `abstract-only` rather than logging. On `ocr-fulltext`, take `printed_page_observed` from a page marker in the OCR text itself or write `"not visible"`; a page number from the search index is not observed.
...
- `source.type`: optional; set it when the source's bibliographic class is evident from the rendered artifact. Exactly one of the eight values in `~/.claude/citations/schema.md` §Source type, never free text. Per-source, like `reliability_basis`: derive once, reuse verbatim across the source's entries. Always set it for conference papers, technical reports, theses, and datasets; those classes have no inference route in the CSL-JSON emitter and render as journal articles without it. Leave it absent when unsure; a wrong type renders worse than no type.
- `source.doi_or_url` holds the canonical DOI when the work has one; the URL you actually retrieved the work from belongs in `retrieval.source_path`. A faculty-hosted or repository PDF URL in `source.doi_or_url` on a formally published article renders into the bibliography where the style wants a DOI or nothing.
...
- **`predatory`** — the venue itself fails the reliability check (content mill, predatory journal, AI-slop repository, unattributed blog standing in for scholarship). The failure is the venue, not the authors: a sound venue whose authors carry a disqualifying interest is `conflict-of-interest`, not `predatory`.
- **`conflict-of-interest`**: the venue passes the reliability check, but the work carries an interest that disqualifies it as evidence for the claim at hand: a declared or undeclared author conflict (funding, founder or board roles, advocacy employment bearing on the claim), or the work is an advocacy organ's position piece being offered as neutral evidence for the claim (a venue that itself fails the §3(a) check stays `predatory`). Include the conflict evidence verbatim in the reason (a printed disclosure statement is ideal). academic-researcher surfaces to {{USER}}; a strong source rejected here is a candidate for use as an object of analysis rather than evidence, with the conflict named in the prose, so when that framing fits, propose it in `### Alternative framings`.
- **`abstract-only`** — only the abstract or first page is accessible; the full work is blocked by something you cannot circumvent. Distinct from `paywalled` in that the blocker may not be a paywall (JSTOR preview limits, scanned excerpt, search-inside snippet windows, etc.).
