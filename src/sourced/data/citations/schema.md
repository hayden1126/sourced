...
- `retrieval.printed_page_observed` — string. The printed page number visible on the rendered page header/footer, OR the literal string `"not visible"` if the header/footer does not show one (scanned image, missing header, digital-only). Verbatim applies to the number, not the typography around it: read the page number off the rendered view (Arabic or Roman), then record it in canonical page form, `"p. N"` for a single page, `"pp. N-M"` for a range (the form every exemplar in this file shows). Required on every entry whose source is paginated.
