...
- **OQ3.** `sourced update` structural diff: implement as a full AST parse of the manifest, or as a simpler section-header-based diff? Phase-2 default: section-header diff using the `marked` lexer (`gfm: false`), matching Claude Code's `claudemd.ts:300` convention. Full AST parse is heavier engineering and phase-3 material.
