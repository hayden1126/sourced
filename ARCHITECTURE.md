...
└── templates/
    ...
    └── voices/
        ...
        ├── technical.md        # Technical-register skeleton (docs, API references, procedural).
        ├── journalistic.md     # Journalistic-register skeleton (news, features, commentary).
        ├── narrative.md        # Narrative-register skeleton (personal essays, reflection, memoir).
        └── hybrid.md           # Register-neutral fallback for blended corpora.
```

`install.sh --global-only` renders into `~/.claude/`. `install.sh` from inside a project renders `<project>/CLAUDE.md`, `<project>/voice.md`, `<project>/style.md`. A prerequisite check (`pdftotext`, `pandoc` ≥ 3.1, `python3`) runs before any per-project install; missing tools abort with apt/brew hints rather than auto-installing. `--global-only` intentionally skips the check, since global-only copies voices/agents/skills/filters/styles into `~/.claude/` and never invokes pandoc, python3, or pdftotext (see [docs/INSTALL.md](./docs/INSTALL.md#prerequisite-check)).
...
1. **Skeletons** (`templates/voices/{academic,casual,technical,journalistic,narrative,hybrid}.md`) — section structure + `## Iron rules` section. Each is a register-specific (or, for `hybrid.md`, register-neutral) calibration template. Copied verbatim into the voice library on global install.
...
- **New voice from scratch.** Copy any of the 6 shipped skeletons (e.g., `templates/voices/academic.md` for an academic-register voice; `templates/voices/casual.md` for casual; `templates/voices/hybrid.md` for register-neutral) → `~/.claude/voice/<name>.md`, edit per-section rules, preserve the `## Iron rules` section verbatim, then `install.sh --voice <name>` from a project.
